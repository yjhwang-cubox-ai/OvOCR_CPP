...
namespace PaddleOCR {
	...
	void Utility::VisualizeBboxes(const cv::Mat& srcimg,
		...
		const std::string& save_path, int& class_id) {
		cv::Mat img_vis;
		srcimg.copyTo(img_vis);
		for (int n = 0; n < ocr_result.size(); n++) {
			/*if (ocr_result[n].score < 0.8)
				continue;*/
			...
			cv::putText(img_vis, ocr_result[n].text, rook_points[0], cv::FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0));

		}

		if (class_id == 0) {
			cv::putText(img_vis, "IDCard", cv::Point (20, 20), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2);
		}
		else {
			cv::putText(img_vis, "Driver", cv::Point(20, 20), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2);
		}
